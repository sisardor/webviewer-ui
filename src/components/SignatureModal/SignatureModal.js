...
class SignatureModal extends React.PureComponent {
  ...
  onSignatureAdded = (e, signatureAnnotation) => {
    ...
    if (this.state.isStoringSignature) {
  ...
  addDefaultSignature = signatureAnnotation => {
    this.signatureTool.freeHandAnnot = window.docViewer.getAnnotationManager().getAnnotationCopy(signatureAnnotation);
